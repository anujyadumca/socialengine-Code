...
class Company_Plugin_Core
{
  public function onStatistics($event)
  {
    ...
    $event->addResponse($select->query()->fetchColumn(0), 'company');
  }
  ...
  public function onUserDeleteBefore($group)
  {
    ...
    if( $payload instanceof User_Model_User ) {
      ...
      // Delete officers
      $listItemTable = Engine_Api::_()->getDbtable('ListItems', 'company');
      ...
      foreach( $listItemTable->fetchAll($listItemSelect) as $listitem ) {
        $list = Engine_Api::_()->getItem('company_list', $listitem->list_id);
        if( !$list ) {
          $listitem->delete();
          continue;
        }
      ...
      // Delete memberships
      $membershipApi = Engine_Api::_()->getDbtable('membership', 'company');
  ...
  public function getActivity($event)
  {
    ...
    if( $user ) {
      $data = Engine_Api::_()->getDbtable('membership', 'company')->getMembershipsOfIds($user);

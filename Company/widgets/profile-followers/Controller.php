 ...
 * @license    http://www.socialengine.com/license/
 */
class Company_Widget_ProfileFollowersController extends Engine_Content_Widget_Abstract
{
  ...
  public function indexAction()
  {
   ...
   $subject = Engine_Api::_()->core()->getSubject('company_group');
    ...
    if( $viewer->getIdentity() && ( $group->isOwner($viewer) || $list->has($viewer) ) ) {
      $this->view->waitingMembers = $waitingMembers = Zend_Paginator::factory($group->followship()->getMembersSelect(false));
    }

    // if not showing waiting members, get full members
    $select = $group->followship()->getMembersObjectSelect();
    if( $search ) {

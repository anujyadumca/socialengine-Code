 ...
 * @license    http://www.socialengine.com/license/
 */
class Company_IndexController extends Core_Controller_Action_Standard
{
  public function init()
  {

    if( !$this->_helper->requireAuth()->setAuthParams('company_group', null, 'view')->isValid() )
        return;

    $id = $this->_getParam('group_id', $this->_getParam('id', null));
    if( $id ) {
      $group = Engine_Api::_()->getItem('company_group', $id);
      if( $group ) {
  ...
  public function browseAction()
  {
    ...
    // Form
//     $this->view->formFilter= $formFilter = new Groupimporter_Form_Filter_Browse();
   ...
   /*  // Populate options
    ...
    // Make paginator
    $this->view->paginator = $paginator = Engine_Api::_()->getItemTable('company')
  ...
  public function createAction()
  {
    ...
    if( !$this->_helper->requireUser->isValid() )
        return;
    if( !$this->_helper->requireAuth()->setAuthParams('company_group', null, 'create')->isValid() )
        return;
    ...
    // Create form
    $this->view->form = $form = new Company_Form_Create();
    ...
    $db = Engine_Api::_()->getDbtable('groups', 'company')->getAdapter();
    $db->beginTransaction();
    ...
    try {
      // Create group
      $table = Engine_Api::_()->getDbtable('groups', 'company');
      ...
      $group->membership()->addMember($viewer)
       ...
       // Add owner as followers edit by vivek
          $group->followship()->addMember($viewer)
          ->setUserApproved($viewer)
          ->setResourceApproved($viewer);
...
//       $action = $activityApi->addActivity($viewer, $group, 'company_create');
//       if( $action ) {
...
//       }
     // Commit
      ...
      // Redirect
      return $this->_helper->redirector->gotoRoute(array('id' => $group->getIdentity()), 'company_profile', true);
    } catch( Exception $e ) {
    	$log->log("Ex:::".$e->getMessage(),Zend_Log::DEBUG);
    	$db->rollback();
  ...
  public function manageAction()
  {
    ...
    // Form
    $this->view->formFilter = $formFilter = new Company_Form_Filter_Manage();
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $membership = Engine_Api::_()->getDbtable('membership', 'company');
    $select = $membership->getMembershipsOfSelect($viewer);
    $select->where('group_id IS NOT NULL');

    $table = Engine_Api::_()->getItemTable('company');

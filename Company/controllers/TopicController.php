 ...
 * @license    http://www.socialengine.com/license/
 */
class Company_TopicController extends Core_Controller_Action_Standard
{
  ...
  public function indexAction()
  {
    ...
    $table = Engine_Api::_()->getDbtable('topics', 'company');
    $select = $table->select()
  ...
  {
    if( !$this->_helper->requireSubject('company_topic')->isValid() ) return;
    if( !$this->_helper->requireAuth()->setAuthParams(null, null, 'view')->isValid() ) return;
    ...
    if( $viewer->getIdentity() ) {
      $topicWatchesTable = Engine_Api::_()->getDbtable('topicWatches', 'company');
    ...
    $table = Engine_Api::_()->getDbtable('posts', 'company');
    $select = $table->select()
      ->where('group_id = ?', $group->getIdentity())
      ->where('topic_id = ?', $topic->getIdentity())
    ...
    if( $canPost && !$topic->closed ) {
      $this->view->form = $form = new Company_Form_Post_Create();
      $form->populate(array(
  ...
  public function createAction()
  {
    ...
    $this->view->group = $group = Engine_Api::_()->core()->getSubject('company_group');
    $this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();

    // Make form
    $this->view->form = $form = new Company_Form_Topic_Create();
    ...
    $topicTable = Engine_Api::_()->getDbtable('topics', 'company');
    $topicWatchesTable = Engine_Api::_()->getDbtable('topicWatches', 'company');
    $postTable = Engine_Api::_()->getDbtable('posts', 'company');
  ...
  {
    $topic = Engine_Api::_()->core()->getSubject('company_topic');
    $viewer = Engine_Api::_()->user()->getViewer();
  ...
  public function renameAction()
  {
    ...
    $this->view->form = $form = new Company_Form_Topic_Rename();
  ...
  public function deleteAction()
  {
    ...
    $this->view->form = $form = new Company_Form_Topic_Delete();
  ...
  {
    $topic = Engine_Api::_()->core()->getSubject();
    $group = Engine_Api::_()->getItem('company', $topic->group_id);
    $viewer = Engine_Api::_()->user()->getViewer();

 ...
 * @license    http://www.socialengine.com/license/
 */
class Company_MemberController extends Core_Controller_Action_Standard
{
	public function init()
	{
		if( 0 !== ($group_id = (int) $this->_getParam('group_id')) &&
				null !== ($group = Engine_Api::_()->getItem('company_group', $group_id)) )
		{
	...
	public function joinAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Join();
		...
		if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
		{
			...
			try
			{
				...
				$action = $activityApi->addActivity($viewer, $subject, 'company_join');
			...
			return $this->_forward('success', 'utility', 'core', array(
					'messages' => array(Zend_Registry::get('Zend_Translate')->_('You are now a member of this company.')),
					'layout' => 'default-simple',
					'parentRefresh' => true,
			));
		}
	...
	/* vivek edits, for company follow */
	public function followAction()
	{
		...
		$viewer = Engine_Api::_()->user()->getViewer();
		$subject = Engine_Api::_()->core()->getSubject();
		$company_id = $this->_getParam ( 'group_id' );
		$followersMembershipTable = Engine_Api::_ ()->getDbtable ( 'followship', 'company' );

		// Make form
		$this->view->form = $form = new Company_Form_Member_Follow();

		// If member is already part of the group
		if( $subject->followship()->isMember($viewer) ) {
			$db = $subject->followship()->getReceiver()->getTable()->getAdapter();
			$db->beginTransaction();
			...
			return $this->_forward('success', 'utility', 'core', array(
					'messages' => array(Zend_Registry::get('Zend_Translate')->_('You are already following this company.')),
					'layout' => 'default-simple',
					'parentRefresh' => true,
			));
		}
		...
		if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
		{
			...
			{
				$formValues = $form->getValues();
				$subject->followship()->addMember ( $viewer )->setUserApproved ( $viewer )->setResourceApproved($viewer);
				// Set the request as handled
				$notification = Engine_Api::_()->getDbtable('notifications', 'activity')->getNotificationByObjectAndType(
						$viewer, $subject, 'group_invite');
				if( $notification )
				...
				// Add activity
				$activityApi = Engine_Api::_()->getDbtable('actions', 'activity');
				$action = $activityApi->addActivity($viewer, $subject, 'company_follow');
			...
			return $this->_forward('success', 'utility', 'core', array(
					'messages' => array(Zend_Registry::get('Zend_Translate')->_('You are now following this company.')),
					'layout' => 'default-simple',
					'parentRefresh' => true,
			));
		}
	...
	public function unfollowAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Unfollow();
		...
		if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
		{
			...
			try
			{
				...
				$subject->followship()->removeMember($viewer);
				$db->commit();
			}
			catch( Exception $e )
			{
			...
			return $this->_forward('success', 'utility', 'core', array(
					'messages' => array(Zend_Registry::get('Zend_Translate')->_('You have successfully unfollow this company.')),
					'layout' => 'default-simple',
					'parentRefresh' => true,
			));
		}
	...
	public function claimAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Claim();
		// Process form
		if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
		{
			try
			{
				//inserting the valaues to the claim table
				$claimTable->insert(
					array(
				...
				//mail to all the admins

			}
			catch( Exception $e )
			{
				$log->log("exception in inserting the data to claim".$e->getMessage(),Zend_Log::DEBUG);
			}

			return $this->_forward('success', 'utility', 'core', array(
					'messages' => array(Zend_Registry::get('Zend_Translate')->_('You have successfully claim this company.')),
					'layout' => 'default-simple',
					'parentRefresh' => true,
			));
		}
	...
	public function requestAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Request();
		...
		if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
		{
			...
			{
				$subject->membership()->addMember($viewer)->setUserApproved($viewer);
				Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $subject, 'company_approve');
				$db->commit();
			}
			catch( Exception $e )
			{
			...
			return $this->_forward('success', 'utility', 'core', array(
					'messages' => array(Zend_Registry::get('Zend_Translate')->_('Company membership request sent')),
					'layout' => 'default-simple',
					'parentRefresh' => true,
			));
		}
	...
	public function cancelAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Cancel();
		...
		if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
		{
			...
			try
			{
				...
				$notification = Engine_Api::_()->getDbtable('notifications', 'activity')->getNotificationByObjectAndType(
						$subject->getOwner(), $subject, 'company_approve');
				if( $notification ) {
					$notification->delete();
				}
			...
			return $this->_forward('success', 'utility', 'core', array(
					'messages' => array(Zend_Registry::get('Zend_Translate')->_('Company membership request cancelled.')),
					'layout' => 'default-simple',
					'parentRefresh' => true,
			));
		}
	...
	public function leaveAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Leave();
		...
		if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) )
		{
			...
			return $this->_forward('success', 'utility', 'core', array(
					'messages' => array(Zend_Registry::get('Zend_Translate')->_('You have successfully left this company.')),
					'layout' => 'default-simple',
					'parentRefresh' => true,
			));
		}
	...
	public function acceptAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Accept();
	...
	public function rejectAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Reject();
	...
	public function promoteAction()
	{
		...
		if( !$group->membership()->isMember($user) ) {
			throw new Company_Model_Exception('Cannot add a non-member as an officer');
		}
	...
	public function demoteAction()
	{
		...
		if( !$group->membership()->isMember($user) ) {
			throw new Company_Model_Exception('Cannot remove a non-member as an officer');
		}
	...
	public function removeAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Remove();
	...
	public function inviteAction()
	{
		...
		// Prepare form
		$this->view->form = $form = new Company_Form_Invite();
	...
	public function approveAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Approve();
	...
	public function editAction()
	{
		...
		// Make form
		$this->view->form = $form = new Company_Form_Member_Edit();

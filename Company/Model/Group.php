 ...
 * @license    http://www.socialengine.com/license/
 */
class Company_Model_Group extends Core_Model_Item_Abstract
{
  ...
  {
    $table = Engine_Api::_()->getItemTable('company_album');
    $select = $table->select()
  ...
  {
    $table = Engine_Api::_()->getItemTable('company_list');
    $select = $table->select()
  ...
  {
    return Engine_Api::_()->getDbtable('categories', 'company')
  ...
  {
  	return Engine_Api::_()->getDbtable('categories', 'company')
  	->find($this->category_id_level_one)->current();
  }

  public function getCategoryLevelTwo()
  {
  	return Engine_Api::_()->getDbtable('categories', 'company')
  	->find($this->category_id_level_two)->current();
  }
  ...
  public function getEventsPaginator()
  {
    ...
    $select = $table->select()->where('parent_type = ?', 'company');
    $select = $select->where('parent_id = ?', $this->getIdentity());
    $log = Engine_Api::_ ()->getApi ( 'core', 'socialapi' )->getLog ();
    $log->log("getEventsPaginator:::".$select,Zend_Log::DEBUG);
  ...
  {
    return new Engine_ProxyObject($this, Engine_Api::_()->getDbtable('membership', 'company'));
  }
  
  public function followship()
  {
  	return new Engine_ProxyObject($this, Engine_Api::_()->getDbtable('followship', 'company'));
  }
  ...
  protected function _delete()
  {
    ...
    // Delete all albums
    $albumTable = Engine_Api::_()->getItemTable('company_album');
    ...
    // Delete all topics
    $topicTable = Engine_Api::_()->getItemTable('company_topic');
    ...
    if (Engine_Api::_()->hasItemType('event'))
    {
      ...
      $eventSelect = $eventTable->select()->where("parent_type = 'company' and parent_id = ?", $this->getIdentity());

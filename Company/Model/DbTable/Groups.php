 ...
 * @license    http://www.socialengine.com/license/
 */
class Company_Model_DbTable_Groups extends Core_Model_Item_DbTable_Abstract
{
	//protected $_type = 'company';
	protected $_name = "company_groups";
    protected $_rowClass = 'Company_Model_Group';
    ...
    public function getGroupSelect($params = array())
    {
        ...
        // Category level one
        if (!empty($params['category_id_level_one'])) {
        	$select->where('category_id_level_one = ?', $params['category_id_level_one']);
        }
        // Category level two
        if (!empty($params['category_id_level_two'])) {
        	$select->where('category_id_level_two = ?', $params['category_id_level_two']);
        }
        ...
        if (!empty($params['companytype'])) {
            $select->where('companytype = ?', $params['companytype']);
        }
        //Full Text
    ...
    {
        $viewer = Engine_Api::_()->user()->getViewer();
        $groupMembershipTable = Engine_Api::_()->getDbtable('membership', 'company');
